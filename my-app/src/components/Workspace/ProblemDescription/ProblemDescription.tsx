...
import { arrayRemove, arrayUnion, doc, getDoc, runTransaction } from 'firebase/firestore';
...
const ProblemDescription:React.FC<ProblemDescriptionProps> = ({problem,_solved}) => {
    const {currentProblem,loading,probelmDifficultyClass, setCurrentProblem} = useGetCurrentProblem(problem.id);
				...
				<div className='px-5'>
					...
					<div className='w-full'>
						...
						{!loading && currentProblem && (
							...
							<div
								className={`${probelmDifficultyClass} inline-block rounded-[21px] bg-opacity-[.15] px-2.5 py-1 text-xs font-medium capitalize `}
							>
							...
							<div className='flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 rounded p-[3px]  ml-4 text-lg transition-colors duration-200 text-dark-gray-6'
								onClick={handleLike} >
								{liked && !updating && <AiFillLike className="text-blue-900"/>}
							...
							onClick={handleDislike}>
								...
								{disliked && !updating && <AiFillDislike className="text-blue-900"/>}
...
function useGetCurrentProblem(problemId:string){
	...
	const [probelmDifficultyClass,setProblemDifficultyClass] = useState<string>("");
...
getCurrentProblem();
	...
	return {currentProblem,loading,probelmDifficultyClass,setCurrentProblem};
}
